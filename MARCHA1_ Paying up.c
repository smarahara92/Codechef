...
int main(void) {
	int t, x, n, m, *wallet, y;
	scanf("%d", &t);
	for(x = 0; x < t; x++) {
		scanf("%d %d", &n, &m);
		wallet = (int *) malloc (n * sizeof(int *));
		for(y = 0; y < n; y++) {
			scanf("%d", &wallet[y]);
		}
		if(findsubsetsum(wallet, m, n, 0, 0)) {
			printf("Yes\n");
		} else {
			printf("No\n");
		}
		//printf("%d\n", findsubsetsum(wallet, m, n, 0, 0));
		free(wallet);
	}
	return 0;
}

int findsubsetsum(int *wallet, int m, int n, int index, int currentsum) {
	int sum = 0;
	for(; index < n; index++) {
		sum = currentsum + wallet[index];
		if(sum == m) {
			return 1;
		}
		if(findsubsetsum(wallet, m, n, index + 1, sum)) {
			return 1;
		}
	}
	return 0;
