...
int getsmallint(int *arr, int length) {
	int smallint = arr[length - 1];
	for(length = length - 2; length > -1; length--) {
		if(smallint > arr[length]) {
			smallint = arr[length];
		}
	}
	return smallint;
...
int isdivisible(int *arr, int length, int factor) {
	for(length = length - 1; length > -1; length--) {
		if(arr[length] % factor != 0) {
			return 0;
		}
	}
	return 1;
...
int gcd(int *arr, int length) {
	int smallint = getsmallint(arr, length);
	int smallfactor = 1, bigfactor = smallint, factor = 1;
	while(smallfactor < bigfactor) {
		if(smallint % smallfactor == 0) {
			bigfactor = smallint / smallfactor;
			if(isdivisible(arr, length, smallfactor)) {
				factor = smallfactor;
			} 
			if(isdivisible(arr, length, bigfactor)){
				return bigfactor;
			}
		}
		smallfactor += 1;
