...
int main(void) {
	int t, n, m, chefn, assistantn, x, y, z, w, alter;
	int *jobs, *chefjobs, *assistantjobs;
	scanf("%d", &t);
	for(x = 0; x < t; x++) {
		scanf("%d %d", &n, &m);
		assistantn = (n - m) / 2;
		chefn = (n - m) - assistantn;
		jobs = (int *) calloc (n, sizeof(int *));
		chefjobs = (int *) malloc (chefn * sizeof(int *));
		assistantjobs = (int *) malloc (assistantn * sizeof(int *));
		for(y = 0; y < m; y++) {
			scanf("%d", &w);
			jobs[w - 1] = 1;
		}
