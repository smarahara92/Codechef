...
int main(void) {
	...
	int hsx, hsy;
	int scx, scy;
	double dchs, dcsc, dhssc;
	int prchs, prcsc, prchssc;
	scanf("%d", &t);
	for(x = 0; x < t; x++) {
		scanf("%d", &r);
		scanf("%d %d", &cx, &cy);
		...
		dchs = DISTANCE(cx, cy, hsx, hsy);
		dcsc = DISTANCE(cx, cy, scx, scy);
		dhssc = DISTANCE(hsx, hsy, scx, scy);
		prchs = (dchs <= r) ? 1 : 0;
		prcsc = (dcsc <= r) ? 1 : 0;
		prchssc = (dhssc <= r) ? 1 : 0;
		...
		if(prchs + prcsc + prchssc == 3) {
			printf("yes\n");
		} else if(prchs + prcsc + prchssc == 2) {
			if((prchs == 0) && (dchs <= 2 * r)) {
				printf("yes\n");
			} else if((prcsc == 0) && (dcsc <= 2 * r)) {
				printf("yes\n");
			} else if (dhssc <= 2 * r) {
				printf("yes\n");
			} else {
