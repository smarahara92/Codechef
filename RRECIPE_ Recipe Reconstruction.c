...
int main(void) {
	int t, x, n, lastindex, y, ok;
	unsigned int possibilities;
	scanf("%d", &t);
	char ingradient[1000000];
	for(x = 0; x < t; x++) {
		scanf("%s", ingradient);
		lastindex = strlen(ingradient) - 1;
		possibilities = 1;
		for(y = 0; lastindex >= y; y++, lastindex--) {
			if(ingradient[y] == '?' && ingradient[lastindex] == '?') {
				possibilities = (possibilities * 26) % 10000009;
			} else if(ingradient[y] == '?' || ingradient[lastindex] == '?') {
				continue;
			} else if(ingradient[y] != ingradient[lastindex]) {
				possibilities = 0;
				break;
